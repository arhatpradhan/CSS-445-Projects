...
public class ArrBag<T>
{
	...
	{ 	// MAKE NEW ARRAY 2xBIG. COPY ALL ELEMS FROM OLD ARR TO NEW ARR
	  	// BE SURE TO REASSIGN REF TO THE NEW 2X ARR INTO THE GLOBAL ARR NAME
			T[] dbl = (T[]) new Object[count * 2];
			//would you have to use this.theArray instead?
	...
	{
		// IM GIVING THE FIRST LINE OF UNION AS FREEBEE. DO THE SAME FOR OTHER METHODS EXCEPT
		...
		for(int i = 0; i < this.size(); i++){
			//add values from the first set into union
			union.add(this.theArray[i]);
		}
		for(int i = 0; i < other.size(); i++){
			//if elements in set2 does not equal something from set1 than add it
